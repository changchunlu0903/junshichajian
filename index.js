// =============================================================
//  军师小剧场 V13.0 - 最终核弹显形版
//  外壳：基于 V4.0 的暴力注入逻辑 (保证100%显示)
//  内核：基于 V12.0 的世界书引擎 (保证功能强大)
// =============================================================

(function() {
    console.log("🚀 军师插件 V13.0 (核弹显形版) 正在强行注入...");

    // 1. 🧹 先清理战场：把旧版本的按钮全部删掉，防止冲突
    const oldIds = ['st-junshi-btn', 'st-entry-btn', 'st-entry-btn-v10', 'st-entry-btn-v11', 'st-entry-btn-v12'];
    oldIds.forEach(id => {
    ...
    // 2. 定义新 ID
    const BOX_ID = 'aiAdvisorBox_v13'; 
    const HEADER_ID = 'advisorHeader_v13';
    const BTN_ID = 'st-nuclear-btn'; // 新名字，防止缓存

    // 存储 Key
    const STORAGE_KEY = 'st_junshi_worldbooks_v11'; // 沿用数据，不用重新导入
    const FAV_KEY = 'st_junshi_favs_v11';

    // 3. 💉 注入 CSS (混合了 V4 的暴力定位 + V12 的美化)
    const style = document.createElement('style');
    style.innerHTML = `
        /* === 悬浮球 (V4 暴力定位风格) === */
        #${BTN_ID} {
            position: fixed !important;
            top: 10px !important;       /* 👈 改回左上角 (V4的位置) */
            left: 10px !important;      /* 👈 改回左上角 */
            width: 50px; 
            height: 50px;
            background: #fff;
        ...
        .advisor-toolbar {
            ...
            background: #fffbf0; border-bottom: 1px solid #ffeaa7;
            align-items: center;
        }
        .advisor-tool-btn {
        ...
        .advisor-action-btn {
            background: #e1f5fe; color: #0288d1; border: 1px dashed #29b6f6;
            border-radius: 6px; width: 100%; padding: 5px; cursor: pointer; margin-top: 5px;
        }

        /* 折叠模式 */
        #${BOX_ID}.collapsed {
            height: 45px !important; min-height: 0 !important; resize: none !important; overflow: hidden !important;
        }
    ...
    function updateStatus() {
        ...
        if(el) el.innerText = lib.length > 0 ? `📚 ${lib.length} 本书` : "📂 空";
    }

    // 5. 🚀 暴力渲染 UI (参考 V4 的写法)
    function renderUI() {
        if (document.getElementById(BTN_ID)) return; // 只有真的没有了才画
        ...
        box.id = BOX_ID;
        box.innerHTML = `
            <div class="header-bar" id="drag-header-v13">
                <span>🤖 军师 (V13核弹版)</span>
                <span style="display:flex; gap:10px;">
                    <span id="st-collapse" style="cursor:pointer;" title="折叠">▼</span>
                    <span id="st-close" style="cursor:pointer;" title="隐藏">×</span>
                </span>
            </div>
        ...
        document.getElementById('advisorSend').onclick = async function() {
            ...
            try {
                ...
                const res = await SillyTavern.generateRaw(prompt, "junshi_v13");
        ...
        // 拖拽逻辑
        const head = document.getElementById('drag-header-v13');
        let isD=false, sX, sY, iL, iT;
        head.addEventListener('mousedown', e => {
            if(e.target!==head && e.target.tagName!=='SPAN') return;
